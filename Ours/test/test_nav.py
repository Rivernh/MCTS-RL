...
actor_list = []
transform = world.get_map().get_spawn_points()[50]  # spwan_points共265个点，选第一个点作为初始化小车的位置
...
while True:
    ...
    transform = vehicle.get_transform()
    spectator.set_transform(carla.Transform(transform.location +
                                            carla.Location(-15 * math.cos((transform.rotation.yaw) / 180 * math.pi),
                                                           -20 * math.sin((transform.rotation.yaw) / 180 * math.pi),
                                                           10),
                                            carla.Rotation(-20, transform.rotation.yaw, transform.rotation.roll)))

    print(f"v:{v}")
    print(f"yaw:{vehicle.get_transform().rotation.yaw}")
